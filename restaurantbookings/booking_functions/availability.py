...
def check_availability(booking_date_time_start, booking_date_time_end, people):
    ...
    # same time frame, and if they are then add them to the
    # existing bookings list
    bookings = Booking.objects.all()
    for booking in bookings:
        if (booking.booking_date_time_start <= booking_date_time_end
           or booking.booking_date_time_end >= booking_date_time_start):
    ...
    # iterate through all table objects to see which are big enough
